...
onMounted(() => {
  ...
  if (tex !== null)
    tareas.value = JSON.parse(tex as string)
  else
    tareas.value = tareasDeEjemplo
})
...
function modificar(indice: number) { // indice = -1 para Cancelar
  ...
  <div class="ms-3">
    ...
    <form @submit.prevent="anadir()" class="mb-3">
      <div class="mb-3">
        ...
        <input v-model="nuevaTarea.texto" name="nuevaTarea" autocomplete="off" class="form-control w-50" />
      </div>
    ...
    <div class="container-fluid">
      <div v-for="(tarea, indice) in tareas" :key="tarea.id" class="row">
        <TodoItem :indice="indice" :editando="editando" :completado="tarea.completado" :texto="tarea.texto"
          @eliminar="eliminar" @modificar="modificar" @modificado="modificado" @completar="cambiarCompletado" />
      </div>
    </div>
  </div>
</template>
